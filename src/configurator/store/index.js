    ...
    reInit(state, data) {
      ...
      walls.forEach((wall, index) => {
        let elements = Object.values(
          state.garageUpdated.walls[wallNames[index]].elements,
        )
...
function updateG(state, data) {
  ...
  if (data.wallId !== undefined && data.eventType == 'update') {
    const elements = Object.values(
      state.garageActual.walls[wallNames[data.wallId]].elements,
    )
    let wallSize =
      data.wallId <= 1
    ...
    if (checkPlacement(data, elements, wallSize)) {
      ...
      for (let i = 0; i < Object.keys(state.garageActual.walls).length; i++) {
        if (
          Object.keys(state.garageActual.walls[wallNames[i]].elements).includes(
            data.name,
          )
        ) {
      ...
      if (fits) {
        for (let i = 0; i < Object.keys(state.garageActual.walls).length; i++) {
          if (
            Object.keys(
              state.garageActual.walls[wallNames[i]].elements,
            ).includes(data.name)
          ) {
            delete state.garageActual.walls[wallNames[i]].elements[data.name]
          }
    ...
    if (data.type === 'fittings') {
      ...
      delete state.garageActual.walls[wallNames[data.wallId]].elements[
        data.name
      ]
    }
    ...
    if (data.roofType) {
      if (data.roofType == 'back') {
        for (
          let i = 0;
          i < Object.values(state.garageActual.walls).length;
          i++
        ) {
...
function fillData(data) {
  ...
  if (data.type) {
    if (data.wallId && data.name) {
      oldData =
        store.getters.getGarage.walls[wallNames[data.wallId]].elements[
          data.name
        ]
    }
    if (oldData) {
