  ...
  if (data.wallId !== undefined && data.eventType == "update") {
    ...
    if (checkPlacement(data, elements, wallSize)) {
      ...
      if (fits) {
        ...
        store.commit("setMsg", { item: data.name, eventName: "xOffsetChange", value: { before: xBefore, after: parseFloat(parseFloat(data.x).toFixed(2)) } });
...
function checkPlacement(item, wallElements, wallSize) {
  ...
  if (item.type !== "gate") {
    ...
    if (item.x + roundTwoDec(item.width + 0.1) > wallSize.x) {
      ...
      store.commit("setMsg", { item: item.name, eventName: "xOffsetChange", value: { before: xBefore, after: parseFloat(parseFloat(item.x).toFixed(2)) } });
    }
    if (item.x < 0.1) {
      xBefore = item.x;
      item.x = 0.1;
    ...
    if (item.y + roundTwoDec(item.height + 0.05) > wallSize.y) {
      ...
      store.commit("setMsg", { item: item.name, eventName: "yOffsetChange", value: { before: yBefore, after: parseFloat(parseFloat(item.x).toFixed(2)) } });
    }
  } else {
    let gateOffset = 0.1;
