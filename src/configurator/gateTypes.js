...
export const doubleDoor = function createDoubleDoor(width, height, material, handleVisible) {
  ...
  const gateDoor = new Mesh(new BoxGeometry(width - frameWidth, height - frameWidth, 0.005), material);
  ...
  if (material.horizontal) {
    ...
    gateDoor.material.map.repeat.set(width - frameWidth, height - frameWidth);
...
export const tiltedWidepanelDoor = function createTiltedWidepanelDoor(width, height, material) {
  ...
  const gateDoor = new Mesh(new BoxGeometry(width - frameWidth, height - frameWidth, 0.005), gateDoorMaterial);
  gateDoor.geometry.groups.forEach((face, index) => {
    face.materialIndex = 0;
    if (index == 5) {
      face.materialIndex = 1;
