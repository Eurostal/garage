...
export default {
  ...
  methods: {
    ...
    changeGarageSize: function ({ width = false, length = false }) {
      ...
      if(width){
        ...
        if(typeof sliderGate1 === 'object'){
          sliderGate1.updateOptions({"range":{"min":0,"max": width}})
        }
        ...
        if(typeof sliderGate2 === 'object'){
          sliderGate2.updateOptions({"range":{"min":0,"max": width}})
        }
      }
    },
    changeGarageWidthEvent: function (e) {
      this.changeGarageSize({
        width: parseFloat(e.target.value.split(" ")[0].replace(",", ".") * 100),
      });
    ...
    changeGarageLengthEvent: function (e) {
      this.changeGarageSize({
        length: parseFloat(e.target.value.split(" ")[0].replace(",", ".") * 100),
      });
